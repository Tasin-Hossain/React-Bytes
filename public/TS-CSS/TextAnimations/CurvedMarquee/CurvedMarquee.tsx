import { useRef, useEffect, useState, useMemo, type CSSProperties, type PointerEvent } from 'react';

const MAX_SPEED = 800;

interface CurvedMarqueeFont {
  fontFamily?: string;
...
export default function CurvedMarquee({
  ...
  const handlePointerDown = (e: PointerEvent<SVGTextElement>) => {
    if (!draggable) return;
  ...
  const handlePointerMove = (e: PointerEvent<SVGTextElement>) => {
    if (!draggable) return;
  ...
  const handlePointerUp = (e: PointerEvent<SVGTextElement>) => {
    if (!draggable) return;
  ...
  return (
    <div
      className={`curved-marquee-container ${className}`}
    ...
    >
      ...
      >
        <text
          ref={measureRef}
          xmlSpace="preserve"
          className="curved-marquee-measure"

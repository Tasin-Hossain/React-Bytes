...
const debounce = (fn, d) => {
  let t;
  return (...a) => { clearTimeout(t); t = setTimeout(() => fn.apply(null, a), d); };
};

export default function ImageCardHover(props) {
  const {
    image, layerImages, layerCount, defaultShape, width, height,
    initialRotation = false, initialBlur = false, initialColor = false,
    initialOpacity = false, initialParallax = false, initial3D = false,
    tiltMax, rounded, panMax, depthStep, scale3D, opacityFalloff,
    moveAmplify, tiltBoost, panBoost, className,
  } = { ...DEFAULTS, ...props };
  ...
  layerRefs.current   = [];
  const addLayerRef   = useCallback(el => { if (el) layerRefs.current.push(el); }, []);
  ...
  useEffect(() => {
    ...
    const duration = 0.8, ease = 'power2.inOut', scaleInterval = 0.06,
      opacityInterval = 0.05, rotationInterval = 15;
    const blurSeq = [0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8, 1.0, 1.3, 1.6];
    const stagger = 0.1, followStrength = 0.15;

    const state = {
      isRotation: rotationOn, isBlur: blurOn, isColor: colorOn,
      isOpacity: opacityOn, isParallax: parallaxOn, is3D: is3DOn,
      isHovered: false, isTransitioning: false,
    };
    ...
    let timeline = null, rect = container.getBoundingClientRect(), rafId = null, pendingMouseEvent = null;

    const onResize = debounce(() => { rect = container.getBoundingClientRect(); }, 50);
    window.addEventListener('resize', onResize);

    const quickTos = layerEls.map((layer, i) => {
      if (i === 0) return null;
      const sv = 1 - scaleInterval * i, mult = sv > 0 ? (1 - sv) * 3 + 0.2 : 1;
      return { layer, mult, xTo: gsap.quickTo(layer, 'x', { duration: 0.6, ease: 'power3' }), yTo: gsap.quickTo(layer, 'y', { duration: 0.6, ease: 'power3' }) };
    }).filter(Boolean);
    ...
    const getColor   = i => {
      ...
      const ci = Math.min(i * 0.15, 1), sat = 1 + ci * 0.5;
      return `grayscale(${1 - ci}) saturate(${sat})`;
    };

    function applyFilters() {
      layerEls.forEach((l, i) => {
        const b = getBlur(i), c = getColor(i);
        let f = '';
        if (b > 0) f += `blur(${b}px) `;
        if (c !== 'none') f += c;
    ...
    function reset2D() {
      ...
      gsap.set(layerEls, { scale: (i, t) => (t === layerEls[0] ? 1 : 0.95), opacity: (i, t) => (t === layerEls[0] ? 1 : 0), rotation: 0, rotationZ: 0, x: 0, y: 0, z: 0 });
    ...
    function createTimeline() {
      ...
      timeline = gsap.timeline({ paused: true }).to(rev, {
        ...
        opacity: (i, t) => { const idx = layerEls.indexOf(t); return idx === 0 ? 1 : getOpacity(idx); },
        rotation: (i, t) => { if (!state.isRotation) return 0; return getRotVal(layerEls.indexOf(t)); },
    ...
    function applyShape(newShape) {
      layerEls.forEach((l, i) => { if (i === 0) return; l.style.clipPath = CLIP_PATH[newShape] || CLIP_PATH.rectangle; });
    }
    ...
    function layout3D(depthFactor = 1) {
      ...
      layerEls.forEach((l, i) => {
        const z = Math.round(i * d) + i * 0.1, s = Math.max(1 - i * cssVarCache.scale3D, 0.35),
          o = compute3DOpacity(i), rotZ = state.isRotation ? getRotVal(i) : 0;
    ...
    function enable3D() {
      ...
      const rev = [...layerEls].reverse();
      gsap.to(rev, {
        scale: (i, t) => { const idx = layerEls.indexOf(t); return idx === 0 ? 1 : Math.max(1 - idx * cssVarCache.scale3D, 0.35); },
        opacity: (i, t) => { const idx = layerEls.indexOf(t); return idx === 0 ? 1 : compute3DOpacity(idx); },
        duration, ease, stagger,
        onComplete: () => { layout3D(); if (state.isHovered) { container.classList.add('lih-is3d-active'); pose3DFlat(); } },
      });
    }
    ...
    function disable3D() {
      ...
      gsap.killTweensOf(stackEl);
      gsap.to(stackEl, { rotationX: 0, rotationY: 0, rotationZ: 0, x: 0, y: 0, z: 0, scale: 1, duration: 0.28, ease: 'power2.out' });
      const rev = [...layerEls].reverse();
      gsap.to(rev, {
        scale: (i, t) => (layerEls.indexOf(t) === 0 ? 1 : 0.95), opacity: (i, t) => (layerEls.indexOf(t) === 0 ? 1 : 0), rotation: 0,
        duration, ease, stagger: -stagger, onComplete: () => { layerEls.forEach(l => (l.style.transform = '')); reset2D(); createTimeline(); },
      });
    }
    ...
    function tiltPan(e) {
      ...
      gsap.to(stackEl, { rotationY: nx * t, rotationX: -ny * t, x: nx * p, y: ny * p, scale: 1, duration: 0.18, ease: 'power2.out' });
    }

    function onEnter() {
      state.isHovered = true;
      if (state.is3D) { container.classList.add('lih-is3d-active'); layout3D(); pose3DFlat(); }
      else if (timeline) { timeline.play(); }
    }

    function onLeave() {
      state.isHovered = false;
      if (state.is3D) { center3D(); container.classList.remove('lih-is3d-active'); }
      else if (timeline && !state.isParallax) { timeline.reverse(); }
      if (state.isParallax) center2D();
      setTimeout(() => { if (!state.isHovered && !state.is3D && !state.isParallax) center2D(); }, duration * 1000);
    }

    function processMouseMove() {
      if (!pendingMouseEvent) { rafId = null; return; }
      const e = pendingMouseEvent; pendingMouseEvent = null; rafId = null;
      if (state.is3D) {
        const inside = e.clientX >= rect.left && e.clientX <= rect.left + rect.width && e.clientY >= rect.top && e.clientY <= rect.top + rect.height;
        inside ? tiltPan(e) : center3D();
        return;
      }
      if (!state.isParallax || !state.isHovered) return;
      const rx = (e.clientX - rect.left) / rect.width - 0.5, ry = (e.clientY - rect.top) / rect.height - 0.5;
      quickTos.forEach(({ xTo, yTo, mult }) => { xTo(rx * 2 * rect.width * followStrength * mult); yTo(ry * 2 * rect.height * followStrength * mult); });
    }
    ...
    controllerRef.current = {
      _changeShape(newShape) {
        ...
        reset2D(); applyShape(newShape); createTimeline();
      ...
      _toggleRotation(v) {
        state.isRotation = v;
        if (state.is3D) { layout3D(); }
        else { if (!v) layerEls.forEach(l => gsap.set(l, { rotation: 0 })); if (timeline) timeline.kill(); createTimeline(); timeline.progress(state.isHovered ? 1 : 0); }
      },
      _toggleBlur(v)    { state.isBlur    = v; applyFilters(); if (state.is3D) layout3D(); },
      _toggleColor(v)   { state.isColor   = v; applyFilters(); if (state.is3D) layout3D(); },
      _toggleOpacity(v) {
        state.isOpacity = v;
        if (state.is3D) { layout3D(); }
        else { if (timeline) timeline.kill(); createTimeline(); timeline.progress(state.isHovered ? 1 : 0); }
      },
      _toggleParallax(v) { state.isParallax = v; if (!v) { center2D(); if (timeline) { state.isHovered ? timeline.play() : timeline.reverse(); } } },
      _toggle3D(v)        { v ? enable3D() : disable3D(); },
      _updateCSSCache:    updateCSSCache,
    };

    reset2D(); createTimeline(); applyShape(shape);
  ...
  return (
    <div className={`inline-flex flex-col items-center gap-7.5 ${className}`}>
      <div
        ...
        style={rootStyle}
      >
        <div ref={stackRef} className="absolute inset-0 transform-flat will-change-transform in-[.lih-is3d-active]:transform-3d">
          {layers.map((_, i) => {
            ...
            return (
              <div
                key={i}
                ref={addLayerRef}
                className={'absolute inset-0 bg-cover bg-center bg-no-repeat backface-hidden will-change-transform ' + (i === 0 ? '' : 'opacity-0 scale-95')}
                style={{ backgroundImage: src ? `url("${src}")` : undefined, borderRadius: `${rounded}px`, clipPath: CLIP_PATH[i === 0 ? 'rectangle' : shape] }}
              />
            );
          })}
        </div>

...
export function SpectraSlider({
  cards = [], numberOfCards = 7, cardWidth = 220, cardHeight = 340, gap = 190,
  perspective = 1200, maxRotation = 35, scaleStep = 0.14, opacityStep = 0.28,
  brightnessStep = 0.35, draggable = true, snap = true, initialIndex = 0,
  className = '', cardClassName = '', onCardClick, autoPlay = false, duration = 20,
  direction = 'vertical', curveIntensity = -20, reverse = false, pauseOnHover = true,
  mouseWheel = true, wheelSmoothness = 0.15, wheelSensitivity = 0.008,
  filterOnHover = false, defaultFiltered = false, entranceAnimation = true,
  entranceDuration = 0.9, entranceStagger = 0.07, entranceEase = 'back.out(1.4)',
  entranceType = 'spin',
}) {
  ...
  const stageRef = useRef(null), cardRefs = useRef([]);
  const offsetRef = useRef(initialIndex);
  const dragRef = useRef({ active: false, startX: 0, startY: 0, startOffset: 0 });
  const tickerFnRef = useRef(null), tickingRef = useRef(false);
  const hoverPausedRef = useRef(false), hoveredRef = useRef(false), mountedRef = useRef(false);
  const wheelTargetRef = useRef(initialIndex), wheelTickerFnRef = useRef(null);
  const wheelTickingRef = useRef(false), wheelSnapTimerRef = useRef(null);
  ...
  const totalRef = useRef(total), gapRef = useRef(gap), maxRotationRef = useRef(maxRotation);
  const scaleStepRef = useRef(scaleStep), opacityStepRef = useRef(opacityStep);
  const brightnessStepRef = useRef(brightnessStep), durationRef = useRef(duration);
  const reverseRef = useRef(reverse), autoPlayRef = useRef(autoPlay), directionRef = useRef(direction);
  const curveIntensityRef = useRef(curveIntensity), wheelSmoothnessRef = useRef(wheelSmoothness);
  ...
  const wrapDiff = (i, offset, total) => { let diff = (i - offset) % total; if (diff > total / 2) diff -= total; if (diff < -total / 2) diff += total; return diff; };
  ...
  const placeCards = useCallback(() => {
    ...
    cardRefs.current.forEach((el, i) => {
      if (!el) return;
      const diff = wrapDiff(i, offsetRef.current, totalRef.current), absDiff = Math.abs(diff);
      const offset = diff * gapRef.current, curveOffset = curveIntensityRef.current * absDiff * absDiff;
      ...
      gsap.set(el, { x: vertical ? curveOffset : offset, y: vertical ? offset : curveOffset, z: -absDiff * 60, rotateY: vertical ? 0 : rotate, rotateX: vertical ? rotate : 0, scale, opacity, zIndex: Math.round(100 - absDiff * 10), filter: `brightness(${brightness})` });
    });
  });

  const stopTicker = () => { if (!tickingRef.current || !tickerFnRef.current) return; gsap.ticker.remove(tickerFnRef.current); tickerFnRef.current = null; tickingRef.current = false; };
  const startTicker = () => {
    if (tickingRef.current) return;
    const dir = reverseRef.current ? -1 : 1;
    tickerFnRef.current = () => { offsetRef.current += (dir * totalRef.current) / (durationRef.current * 60); offsetRef.current = ((offsetRef.current % totalRef.current) + totalRef.current) % totalRef.current; placeCards(); };
    gsap.ticker.add(tickerFnRef.current); tickingRef.current = true;
  };
  const maybeStartTicker = () => { if (autoPlayRef.current && !hoverPausedRef.current && !dragRef.current.active && !wheelTickingRef.current) startTicker(); };
  ...
  const snapToNearest = () => {
    ...
    gsap.to(offsetRef, { current: target, duration: 0.5, ease: 'power3.out', onUpdate: placeCards, onComplete: () => { offsetRef.current = ((offsetRef.current % totalRef.current) + totalRef.current) % totalRef.current; wheelTargetRef.current = offsetRef.current; placeCards(); maybeStartTicker(); } });
  };
  ...
  const onPointerUp = () => { if (!dragRef.current.active) return; dragRef.current.active = false; if (snap) { snapToNearest(); } else { maybeStartTicker(); } };
  ...
  const stopWheelTicker = () => { if (!wheelTickingRef.current || !wheelTickerFnRef.current) return; gsap.ticker.remove(wheelTickerFnRef.current); wheelTickerFnRef.current = null; wheelTickingRef.current = false; };
  const startWheelTicker = () => {
    ...
    wheelTickerFnRef.current = () => {
      ...
      if (Math.abs(diff) < 0.001) { offsetRef.current = wheelTargetRef.current; placeCards(); stopWheelTicker(); if (snap) { clearTimeout(wheelSnapTimerRef.current); wheelSnapTimerRef.current = setTimeout(snapToNearest, 100); } else { maybeStartTicker(); } return; }
      offsetRef.current += diff * wheelSmoothnessRef.current; placeCards();
    };
    gsap.ticker.add(wheelTickerFnRef.current);
  };

  useEffect(() => {
    const el = stageRef.current; if (!el || !mouseWheel) return;
    const handler = e => {
  ...
  const handleCardClick = (card, index) => {
    ...
    else { stopTicker(); stopWheelTicker(); const target = offsetRef.current + diff; gsap.to(offsetRef, { current: target, duration: 0.5, ease: 'power3.out', onUpdate: placeCards, onComplete: () => { offsetRef.current = ((offsetRef.current % totalRef.current) + totalRef.current) % totalRef.current; wheelTargetRef.current = offsetRef.current; placeCards(); maybeStartTicker(); } }); }
  };
  ...
  const stageStyle = isVertical ? { height: '100%', minHeight: cardHeight + 60, perspective } : { height: cardHeight + 60, perspective };

  return (
    <div ref={stageRef} className={`relative w-full flex items-center justify-center select-none ${className}`} style={stageStyle}>
      <div className="relative" style={{ width: cardWidth, height: cardHeight, transformStyle: 'preserve-3d' }}>
        {activeCards.map((card, i) => (
          <div key={card.id ?? i} ref={el => (cardRefs.current[i] = el)} className={`absolute top-0 left-0 rounded-2xl overflow-hidden bg-neutral-900 ${cardClassName}`} style={{ width: cardWidth, height: cardHeight, willChange: 'transform' }}
            onClick={() => handleCardClick(card, i)} onMouseDown={onCardPointerDown} onTouchStart={onCardPointerDown} onMouseEnter={() => handleCardMouseEnter(i)} onMouseLeave={() => handleCardMouseLeave(i)}>
            {card.image && <img src={card.image} alt={card.label ?? ''} className="w-full h-full object-cover pointer-events-none" draggable={false} style={{ filter: filteredCards.has(i) ? FILTER_STYLE : 'none' }} />}

...
function buildGlyphAtlas(chars, fill = 0.5, cellPx = 64) {
  ...
  list.forEach((ch, i) => {
    ...
    // Dilate by re-stamping the glyph around a small ring; radius scales
    // with `fill`, so fill=0 stays thin (single stamp) and fill=1 looks
    // bold/solid.
    if (offsetR > 0.01) {
...
// Same raymarched ribbon field as before, but each grid cell now samples
// one character glyph from uGlyphAtlas, chosen by that cell's brightness
// level, and tinted with the ribbon color. uCellSize = glyph/cell size
// in px. Glyph thickness itself is baked into the atlas via uCellFill
// (see buildGlyphAtlas), not done in the shader.
const FRAG = /* glsl */ `
...
export default function AsciiPlasmaWave(props) {
  ...
  // Rebuild the glyph atlas whenever the character set OR the fill
  // (stroke weight) changes — thickness is baked into the texture, not

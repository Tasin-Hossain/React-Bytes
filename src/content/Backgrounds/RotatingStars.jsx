...
const FRAG_SRC = `
  ...
  void main() {
    ...
    // ring spacing shrinks as radius grows -> more rings visible (denser field).
    // domain-repeat via floor() makes the field infinite: no hard edge no
    // matter how far the center is shifted off-screen.
    ...
    for (int k = -1; k <= 1; k++) {
      ...
      // All rings now rotate at the same angular speed (only randomized by a
      // per-ring phase offset). Real long-exposure star trails all sweep
      // together around the same pole — dividing speed by idx (old code) gave
      // every ring a wildly different rate and was the main cause of the
      // chaotic, unsynchronized look.
      float angOffset = uTime * uSpeed + h3 * 6.2831853;
      float ang = mod(a - angOffset, 6.2831853);

      // Cap rounding is now scaled relative to each arc's own length, so the
      // effect is clearly visible across the whole 0-1 range (previously it
      // used a fixed tiny constant that was negligible next to typical arc
      // lengths, making uRoundness barely do anything). Clamped below half
      // the arc length to avoid a degenerate/undefined smoothstep.
      ...
      // uThickness alone sets the line's solid core width, with a tiny fixed
      // crispness epsilon for a clean edge.
...
export default function RotatingStars({
  ...
  return (
    <div className={`relative w-full h-full ${className}`} style={{ minHeight: "300px" }}>
      ...
      />
      {error && (
        <div className="absolute inset-0 flex items-center justify-center text-red-400 text-xs font-mono p-4">

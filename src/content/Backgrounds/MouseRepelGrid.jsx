...
const MouseRepelGrid = ({
  ...
  // Animation mode
  animationMode         = "none",   // "none" | "breathing" | "shimmer" | "pulse-glow" | "auto-ripple"
  animationSpeed        = 1,
  ...
  lineWidth             = 1,
  glowIntensity         = 0.7,
  glowBlur              = 0,        // canvas shadowBlur for soft glow
  ...
  gradientDirection     = "horizontal", // "horizontal" | "vertical" | "diagonal" | "radial"
  rainbowSpeed          = 0.2,
  ...
  useEffect(() => {
    ...
    const buildGrid = () => {
      ...
      for (let j = 0; j < pY; j++) {
        for (let i = 0; i < pX; i++) {
          ...
          nodes.push({ x:bx, y:by, bx, by, vx:0, vy:0, glow:0,
    ...
    const animate = () => {
      ...
      for (const n of nodes) {
        // idle wave
        const waveOff = waveAmplitude > 0
          ? Math.sin((n.bx/W)*Math.PI*2 + t*waveSpeed*2)*waveAmplitude : 0;
        ...
        if (animationMode==="breathing") {
          ...
          baseX = cx+(n.bx-cx)*scale;
          baseY = cy+(n.by-cy)*scale;
        }
        ...
        if (animationMode==="pulse-glow") {
          ...
          if (age<1.2) {
            const ringR=age*Math.max(W,H)*0.6;
            const pd=Math.sqrt((bx-cx)**2+(by-cy)**2);
      ...
      for (let j=0; j<pY; j++) {
        for (let i=0; i<pX; i++) {
          ...
          const lw      = lineWidth + glow*1.5;

          ctx.strokeStyle = `rgba(${r},${g},${b},${alpha})`;
          ctx.lineWidth   = lw;

          if (glowBlur > 0) {
            ctx.shadowColor = `rgba(${glowRgb[0]},${glowRgb[1]},${glowRgb[2]},${glow})`;
            ctx.shadowBlur  = glowBlur * glow;
          } else {
            ctx.shadowBlur = 0;
          }

          if (i < pX-1) {
            const right=nodes[idx(i+1,j)];
            ctx.beginPath(); ctx.moveTo(n.x,n.y); ctx.lineTo(right.x,right.y); ctx.stroke();
          }
          if (j < pY-1) {
            const down=nodes[idx(i,j+1)];
            ctx.beginPath(); ctx.moveTo(n.x,n.y); ctx.lineTo(down.x,down.y); ctx.stroke();
          }
          if (diagonals && i<pX-1 && j<pY-1) {
            const diag=nodes[idx(i+1,j+1)];
            ctx.beginPath(); ctx.moveTo(n.x,n.y); ctx.lineTo(diag.x,diag.y); ctx.stroke();
          }
        }
      }

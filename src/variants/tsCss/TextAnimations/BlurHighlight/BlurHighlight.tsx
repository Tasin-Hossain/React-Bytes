...
export default function BlurHighlight({
  ...
  return (
    <Tag
      ref={wrapperRef}
      className={`mr-highlight ${className}`}
      style={{ color: textColor, "--mr-mask-color": maskColor, "--mr-bg-position": bg.position, "--mr-bg-size": bg.from, ...style } as CSSProperties}
    >
      {segments.map((seg, i) =>
        seg.hi ? (
          <span className="mr-pill" key={i}>
            {seg.text}
          </span>
        ) : (
      ...
      <style>{`
        ...
        .mr-pill {
          ...
          background-image: linear-gradient(var(--mr-mask-color), var(--mr-mask-color));
          background-repeat: no-repeat;
          background-position: var(--mr-bg-position);
          background-size: var(--mr-bg-size);
        }
      `}</style>
    </Tag>
  );

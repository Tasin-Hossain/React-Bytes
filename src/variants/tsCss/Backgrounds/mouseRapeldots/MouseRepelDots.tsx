...
const MouseRepelDots = memo(({
  ...
  const propsRef = useRef<MouseRepelDotsProps>({});
  propsRef.current = {
  ...
  useEffect(() => {
    ...
    let mouse: { x: number; y: number } = { x: -9999, y: -9999 };
    let dots: Dot[] = [];
    let animId: number;
    let frameCount = 0;
    let cols = 0, rows = 0;
    ...
    function makeDots() {
      const { dotRadius, dotSpacing } = propsRef.current as Required<MouseRepelDotsProps>;
      ...
      for (let j = 0; j < rows; j++) {
        for (let i = 0; i < cols; i++) {
          ...
          dots.push({ ox, oy, sx: ox, sy: oy, x: ox, y: oy, vx: 0, vy: 0, col: i, row: j, sparkleSeed: Math.random() * 1000, sparklePhase: Math.random() * TWO_PI });
        }
      }
    }

    function getSparkleIntensity(d: Dot, t: number, mode: SparkleMode, density: number): number {
    ...
    function draw() {
      if (remakeRef.current) { remakeRef.current = false; makeDots(); }
      ...
      const {
        ...
        bulgeOnly, bulgeStrength, waveAmplitude,
        sparkleMode, sparkleColor, sparkleSize, sparkleSpeed, sparkleDensity,
      } = propsRef.current as Required<MouseRepelDotsProps>;
      ...
      dots.forEach((d) => {
        ...
        let r:     number = dotRadius + energy * (maxDotSize - dotRadius);
        let alpha: number = 0.35 + energy * 0.65;
        let color: string | CanvasGradient = gradFill ?? (energy > 0.6 ? dotColorHot : energy > 0.25 ? dotColorMid : dotColor);

        const s = getSparkleIntensity(d, sparkleT, sparkleMode as SparkleMode, sparkleDensity);
        if (s > 0) {
    ...
    function onResize() { setCanvasSize(); makeDots(); }
  ...
  const style = {
    wrapper: { position: 'relative' as const, width: '100%', height: '100%' },
    canvas:  { display: 'block', width: '100%', height: '100%', pointerEvents: 'none' as const },
  };

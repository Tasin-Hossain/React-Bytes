...
export interface FluidCursorEffectProps {
  /** Simulation grid resolution (lower = faster, blockier fluid). */
  simResolution?: number;
  /** Resolution of the dye/color texture that gets rendered on screen. */
  dyeResolution?: number;
  /** How quickly the color dye fades out over time. */
  densityDissipation?: number;
  /** How quickly the velocity field slows down over time. */
  velocityDissipation?: number;
  /** Pressure retained between simulation steps (0-1). */
  pressure?: number;
  /** Jacobi iterations used to solve the pressure field. */
  pressureIterations?: number;
  /** Strength of vorticity confinement (swirliness). */
  curl?: number;
  /** Radius (relative to canvas) of each pointer splat. */
  splatRadius?: number;
  /** Force multiplier applied to pointer movement. */
  splatForce?: number;
  /** Enable the simple normal-based shading pass. */
  shading?: boolean;
  /** How fast the auto-generated hue cycles, in Hz-like units. */
  colorUpdateSpeed?: number;
  /** Fixed hex color (e.g. "#38bdf8") to use instead of the auto rainbow. */
  color?: string | null;
  /** Extra class name(s) applied to the outer container. */
  className?: string;
}

function hexToRgb01(hex?: string | null): RGBColor | null {
  if (!hex) return null;
...
function FluidCursorEffect({
  ...
  const updateKeywordsRef = useRef<(() => void) | null>(null);
  ...
  useEffect(() => {
    ...
    let pointers = [{
      ...
      deltaX: 0, deltaY: 0, down: false, moved: false, color: { r: 0, g: 0, b: 0 } as RGBColor,
    }];
    ...
    const supportRenderTextureFormat = (gl: WebGLRenderingContext | WebGL2RenderingContext, internalFormat: number, format: number, type: number): boolean => {
    ...
    const splat = (x: number, y: number, dx: number, dy: number, color: RGBColor) => {
      splatProgram.bind();
    ...
    const correctDeltaX = (delta: number): number => { const a = canvas.width / canvas.height; if (a < 1) delta *= a; return delta; };
    const correctDeltaY = (delta: number): number => { const a = canvas.width / canvas.height; if (a > 1) delta /= a; return delta; };

    const updatePointerMoveData = (pointer: (typeof pointers)[number], posX: number, posY: number, color: RGBColor) => {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1 - posY / canvas.height;
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full min-h-75 bg-transparent text-white rounded-xl">
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden bg-transparent ${className}`}>
      ...
      {!isInitialized && (
        <div className="absolute top-4 right-4 bg-blue-500/20 text-blue-200 px-3 py-1.5 rounded-lg border border-blue-500/30 text-xs">
          Initializing WebGL...
        </div>
      )}
    </div>

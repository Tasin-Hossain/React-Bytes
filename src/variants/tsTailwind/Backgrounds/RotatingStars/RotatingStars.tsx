  ...
  useEffect(() => {
    ...
    try {
      ...
      function render(now: number) {
        ...
        glContext.uniform2f(uCenter, x, y);
        glContext.uniform1f(uRadius, radius);
        glContext.uniform1f(uSpeed, speed);
        glContext.uniform1f(uThickness, thickness);
        glContext.uniform1f(uRoundness, roundness);
        glContext.uniform1f(uOpacity, opacity);
        glContext.uniform3f(uColor, rgb[0], rgb[1], rgb[2]);
  ...
  return (
    <div className={`relative w-full h-full ${className}`} style={{ minHeight: '300px' }}>
      <canvas
        ...
        style={{ filter: blur > 0 ? `blur(${blur}px)` : 'none' } as CSSProperties}
      />
      {error && (

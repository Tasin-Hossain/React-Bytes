...
function ScrollStack(props) {
  ...
  const style = {
    ...
    date: { fontSize: dateSize, color: '#9ca3af' },
    title: { fontSize: titleSize, color: '#111827', marginTop: '0.5rem', lineHeight: 1.4, fontWeight: 500 },
    desc: { fontSize: descSize, color: '#6b7280', marginTop: '0.25rem', lineHeight: 1.4 },
    spacer: { flex: 1 },
    footer: {
      display:        'flex',
      alignItems:     'center',
  ...
  return (
    <div ref={scrollerRef} className="stack-scroller" style={style.scroller}>
      <div style={style.inner}>
        {items.map((card, i) => (
          <div key={i} ref={el => (cardRefs.current[i] = el)} style={{ ...style.card, zIndex: i + 1 }}>
            <div style={style.row}>

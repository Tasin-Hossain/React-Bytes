...
export default function ImageCardHover(props) {
  ...
  useEffect(() => {
    ...
    let rafId = null, pendingMouseEvent = null;
    ...
    function enable3D() {
      ...
      gsap.to(rev, {
        ...
        onComplete: () => {
          layout3D();
          if (state.isHovered) { container.classList.add('lih-is3d-active'); pose3DFlat(); }
        },
      });
    }
    ...
    function disable3D() {
      ...
      gsap.to(rev, {
        ...
        rotation: 0, duration, ease, stagger: -stagger,
        onComplete: () => { layerEls.forEach(l => (l.style.transform = '')); reset2D(); createTimeline(); },
      });
    }
    ...
    controllerRef.current = {
      ...
      _toggleParallax(v) {
        state.isParallax = v;
        if (!v) { center2D(); if (timeline) { state.isHovered ? timeline.play() : timeline.reverse(); } }
      },
  ...
  const style = {
    outer: { display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: '1.875rem' },
  };

...
const MouseRepelGrid = ({
  ...
  useEffect(() => {
    ...
    const colorNorm = (color) => parseToNorm(resolveColor(color));
    const hexNorm   = (hex)   => parseToNorm(hex);
    ...
    const gridUniforms = {
      ...
      uLW:  { value: lineWidth },
      uGC:  { value: colorNorm(glowColor) },
    };
    const vigUniforms = {
      uBg:       { value: colorNorm(backgroundColor) },
      uStrength: { value: transparent ? 0 : vignetteStrength },
    };
    ...
    const baseColorFor = (bx, by, W, H, t) => {
    ...
    const buildGrid = () => {
      ...
      const pX   = Math.floor(W / gridSpacing) + 1;
      const pY   = Math.floor(H / gridSpacing) + 1;
      const offX = (W - (pX-1)*gridSpacing) / 2;
      const offY = (H - (pY-1)*gridSpacing) / 2;
      const cx = W/2, cy = H/2;
      ...
      for (let j = 0; j < pY; j++) {
        for (let i = 0; i < pX; i++) {
          let bx = offX + i*gridSpacing;
          let by = offY + j*gridSpacing;
          if (curveStrength > 0) {
            const dx=bx-cx, dy=by-cy;
            const d=Math.sqrt(dx*dx+dy*dy)/maxDist;
            const f=1+(curveStrength/60)*0.4*d;
            bx=cx+dx*f; by=cy+dy*f;
          }
      ...
      const total  = pX * pY;
      const posD   = new Float32Array(total * 2);
      const glowD  = new Float32Array(total);
      const colorD = new Float32Array(total * 3);
      const idx    = (i, j) => j*pX+i;
      const lineIdx = [];
      for (let j = 0; j < pY; j++) {
        for (let i = 0; i < pX; i++) {
      ...
      S.vignetteMesh = new Mesh(gl, { mode: gl.TRIANGLES, geometry: new Geometry(gl, {
        aPos: { size:2, data:vigPos },
      }), program: vigProg });
    };
    ...
    const animate = () => {
      ...
      const posD   = S.lineMesh.geometry.attributes.aPos.data;
      const glowD  = S.lineMesh.geometry.attributes.aGlow.data;
      const colorD = S.lineMesh.geometry.attributes.aColor.data;
      ...
      for (let k = 0; k < nodes.length; k++) {
        ...
        let ambX=0, ambY=0;
        if (ambientNoise && ambientNoiseAmplitude > 0) {
          ambX = Math.sin(t*ambientNoiseSpeed + n.noisePhaseX)*ambientNoiseAmplitude;
          ambY = Math.cos(t*ambientNoiseSpeed*0.8 + n.noisePhaseY)*ambientNoiseAmplitude;
        }

        let baseX=n.bx, baseY=n.by;
        if (animationMode === 'breathing') {
          const scale = 1+Math.sin(t*animationSpeed)*0.04*animationIntensity;
          baseX=S.cx+(n.bx-S.cx)*scale;
          baseY=S.cy+(n.by-S.cy)*scale;
        }

        const bx=baseX+ambX, by=baseY+waveOff+ambY;
        let tx=bx, ty=by;
        const ddx=bx-mouse.x, ddy=by-mouse.y;
        const dist=Math.sqrt(ddx*ddx+ddy*ddy);
        n.glow=0;

        if (dist < repelRadius && dist > 0.01) {
          const s=1-dist/repelRadius;
          n.glow=s*glowIntensity;
          const push=s*force;
          tx+=(ddx/dist)*push;
          ty+=(ddy/dist)*push;
        }

        if (pulseOnClick && S.pulseStart > -5) {
          const age=t-S.pulseStart;
          if (age >= 0 && age < 1.2) {
            const ringR=age*Math.max(W,H)*0.6;
            const pd=Math.sqrt((bx-pulseCenter.x)**2+(by-pulseCenter.y)**2);
            const band=Math.exp(-(((pd-ringR)*0.05)**2))*(1-age/1.2);
            n.glow=Math.max(n.glow, band*glowIntensity);
            const ang=Math.atan2(by-pulseCenter.y, bx-pulseCenter.x);
            tx+=Math.cos(ang)*band*8;
            ty+=Math.sin(ang)*band*8;
          }
        }

        if (animationMode === 'pulse-glow') {
          const pulse=(Math.sin(t*animationSpeed*2)*0.5+0.5)*glowIntensity*0.6*animationIntensity;
          n.glow=Math.max(n.glow, pulse);
        } else if (animationMode === 'shimmer') {
          const head=(t*animationSpeed*0.3)%1.4-0.2;
          const nx=n.bx/W;
          const band=Math.max(0, 1-Math.abs(nx-head)/0.08)*animationIntensity;
          n.glow=Math.max(n.glow, band*glowIntensity);
        } else if (animationMode === 'auto-ripple') {
          const period=3/Math.max(animationSpeed,0.01);
          const age=t%period;
          if (age < 1.2) {
            const ringR=age*Math.max(W,H)*0.6;
            const pd=Math.sqrt((bx-S.cx)**2+(by-S.cy)**2);
            const band=Math.exp(-(((pd-ringR)*0.05)**2))*(1-age/1.2)*animationIntensity;
            n.glow=Math.max(n.glow, band*glowIntensity);
          }
        }
        ...
        n.x+=(tx+n.vx-n.x)*easeSpeed;
        n.y+=(ty+n.vy-n.y)*easeSpeed;

        posD[k*2]=n.x; posD[k*2+1]=n.y;
        glowD[k]=n.glow;
    ...
    const onClick  = (e) => {
      ...
      S.pulseCenter.x=e.clientX-r.left; S.pulseCenter.y=e.clientY-r.top;
      S.pulseStart=(performance.now()-S.t0)/1000;
    };

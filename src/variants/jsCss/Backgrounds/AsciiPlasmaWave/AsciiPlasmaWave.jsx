  ...
  list.forEach((ch, i) => {
    ...
    if (offsetR > 0.01) {
      for (let s = 0; s < ringSteps; s++) {
        ...
        ctx.fillText(ch, cx + Math.cos(angle) * offsetR, cy + Math.sin(angle) * offsetR);
      }
    }
  });
...
export default function AsciiPlasmaWave(props) {
  ...
  useEffect(() => {
    ...
    if (!textureRef.current) {
      textureRef.current = new Texture(gl, {
        image: canvas, generateMipmaps: false,
  ...
  useEffect(() => {
    ...
    const glyphTexture = new Texture(gl, {
      image: atlasCanvas, generateMipmaps: false,
      minFilter: gl.LINEAR, magFilter: gl.LINEAR,
      wrapS: gl.CLAMP_TO_EDGE, wrapT: gl.CLAMP_TO_EDGE,
    });
  ...
  return <div ref={containerRef} style={style.container} className={classes} />;
}